...
namespace AppVentus\Awesome\ShortcutsBundle\Date;
...
class MomentFormatConverter
{
    ...
    public function convert($format)
    {
        if (!array_key_exists($format, $this->phpMomentMapping)) {
            throw new \RuntimeException(sprintf("PHP Date format '%s' is not a convertible moment.js format; please add it to the 'Sonata\CoreBundle\Date\MomentFormatConverter' class by submitting a pull request if you want it supported.", $format));
        }
